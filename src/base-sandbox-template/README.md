# Base Sandbox Dev Container (base-sandbox-template)

Hardened Debian trixie dev container for coding agents: node 24, OS Python/pip/pipx, uv, github-cli, copilot-cli, claude-code, az-cli, otel-collector, speckit, superfile and herdr - baked into a pre-built image without Dev Container Features. No sudo, no SSH host keys in the image, no host sockets or credentials passed in.

## Options

| Options Id | Description | Type | Default Value |
|-----|-----|-----|-----|
| sshdPort | Port sshd listens on inside the container. Read at container start, so it can be changed without rebuilding the image. | string | 2222 |
| startSshd | Start sshd on every container start. Set to false to leave the port closed; the host keys are then generated the first time you start it by hand. | string | true |
...
_Note: This file was auto-generated from the [devcontainer-template.json](https://github.com/bugrasan/devcontainer-base-ai/blob/main/src/base-sandbox-template/devcontainer-template.json).  Add additional notes to a `NOTES.md`._
